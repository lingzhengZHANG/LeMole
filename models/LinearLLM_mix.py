...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.num_predictions_F = configs.num_F_expert
        self.num_predictions_T = configs.num_T_expert
        self.num_predictions = self.num_predictions_T + self.num_predictions_F
        self.seq_len = configs.seq_len
        self.pred_len = configs.pred_len
        self.channels = configs.enc_in
        if configs.seq_len==512:
            self.interval = [512,336,192,96]#2**(np.ceil(math.log2(self.num_predictions))) 
        if configs.seq_len==1024:   
            self.interval = [1024,672,512,336,192,96]
        self.interval_F = self.interval[0:self.num_predictions_F]
        self.interval_T = self.interval[0:self.num_predictions_T]
        
        self.length_ratio = [(n + self.pred_len)/self.seq_len for n in self.interval_F]
        
        self.dominance_freq=[int(self.interval_F[i] // self.configs.base_T + 1) * self.configs.H_order + 10 for i in range(self.num_predictions_F)]
        
        self.Linear_T = nn.ModuleList([
            nn.Linear(n, configs.pred_len) for n in self.interval_T
        ])
        self.Linear_F = nn.ModuleList([
            nn.Linear(self.dominance_freq[i], int(self.dominance_freq[i]*self.length_ratio[i])).to(torch.cfloat) for i in range(self.num_predictions_F)
    ...
    def forward(self, x,  x_mark, x_mark_enc, static_prompt_embd, return_gating_weights=False, return_seperate_head=False):
        ...
        for idx, proj in enumerate(self.Linear_T):
            pred[:, y_shape[1]*idx:y_shape[1]*(idx+1), :] = proj(x[:, -self.interval_T[idx]:, :].transpose(1, 2)).transpose(1, 2)
        
        for idx, proj in enumerate(self.Linear_F):
            # pred[:, y_shape[1]*idx:y_shape[1]*(idx+1), :] = proj(x[:, -self.interval[idx]:, :].transpose(1, 2)).transpose(1, 2)         
            low_specx = torch.fft.rfft(x[:, -self.interval_F[idx]:, :], dim=1)
            ...
            low_specxy = torch.zeros([low_specxy_.size(0),int((self.interval_F[idx]+self.pred_len)/2+1),low_specxy_.size(2)],dtype=low_specxy_.dtype).to(low_specxy_.device)
            low_specxy[:,0:low_specxy_.size(1),:]=low_specxy_ # zero padding
            low_xy=torch.fft.irfft(low_specxy, dim=1)
            pred[:, y_shape[1]*(self.num_predictions_T+idx):y_shape[1]*(self.num_predictions_T+idx+1), :]=(low_xy * self.length_ratio[idx])[:, -self.pred_len:] # energy compemsation for the length change
